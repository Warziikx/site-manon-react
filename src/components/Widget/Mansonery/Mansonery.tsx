...
export const Mansonery: React.FC<MansoneryProps> = ({ data }) => {
	return (
		<div
			className={`grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-${data.length} gap-4 py-8 px-16 md:px-32 lg:px-32`}
		>
				...
				<div className="grid gap-4">
					{mans.images
						...
						.map((item: IMansoneryObj) => {
							return (
								<div>
									<img className="h-auto rounded-lg" src={"/" + item.image} alt="" />
								</div>
							);
						})}
				</div>
