...
export const IllustrationFeatures: React.FC<NumberFeaturesPropos> = ({ data }) => {
	return (
		<div className="space-y-16 py-16 lg:px-32">
			<h2 className="text-center font-title text-4xl font-semibold text-ternary">{data.title}</h2>
			<div className="grid grid-cols-1 gap-16 md:grid-cols-2">
				{data.list.map((feature: IPrestationObj, index: number) => (
					<div key={index} className="flex flex-col items-center">
						<img className="max-w-md" src={feature.image} alt={`Illustration de la présation ${data.title}`} />
						<Link
							resetScroll={true}
							to={feature.link}
